...
class Node:
    ...
    def update_neighbours(self, nodes):
        ...
        if (
            self.row != 0 and nodes[self.row - 1][self.col].get_state() != "Barrier"
        ):  # UP
            self.neighbours.append(nodes[self.row - 1][self.col])
        if (
            self.row < self.rows - 1
            and nodes[self.row + 1][self.col].get_state() != "Barrier"
        ):  # DOWN
            self.neighbours.append(nodes[self.row + 1][self.col])
        if (
            self.col != 0 and nodes[self.row][self.col - 1].get_state() != "Barrier"
        ):  # LEFT
            self.neighbours.append(nodes[self.row][self.col - 1])
        if (
            self.col < self.rows - 1
            and nodes[self.row][self.col + 1].get_state() != "Barrier"
        ):  # RIGHT
            self.neighbours.append(nodes[self.row][self.col + 1])
...
def a_star(draw, start_node: Node, end_node, nodes):
    borders = []
    visited = []
    finished = False
...
def draw_board(win, rows, width):
    ...
    for x in range(1, rows):
        pygame.draw.line(
            win, LINE_COLOR, (gap * x, 0), (gap * x, width), 2
        )  # Last item == line.weight = 1 by default
